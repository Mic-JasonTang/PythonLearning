...
def f(x):
	...
	i = x < 0
	#  a negative integer power will raise a ValueError.
	y[i] = np.power(-x[i], -x[i])
	return y
...
if __name__ == '__main__':
	...
	# # # linspace函数通过指定起始值、终止值和元素个数来创建数组(等差数列)，缺省包括终止值
	...
	# print '去重后：', b
	#
	# # # 4.2.2 二维数组的去重，结果会是预期的么？  将其转换成了一维列表再进行了去重
	...
	# t = np.arange(len(x))
	# plt.hist(x, 50, color='k', alpha=0.5, label=u'均匀分布')
	...
	# plt.legend(loc='upper left') # loc=1
	# plt.grid()
	# plt.show()
	...
	# mu = 2
	# sigma = 3
	# # np.random.randn()返回的是服从标准正态分布的随机数
	# data = mu + sigma * np.random.randn(1000)
	# print data
	# h = plt.hist(data, 30, normed=1, color='#00FFFF')
	# x = h[1]
